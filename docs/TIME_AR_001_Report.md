...
**Word Count:** ~32,000
**Changelog:**
- v3.1.2 → v3.2: Adds evidence classification framework (§2.11): evidence modes, computational roles, and authority file designations across all 13 raw CSVs. Adds source registry (103 citations with DOIs/URLs), organism time-budget provenance file, field-level evidence matrix, and D-value depth provenance file. Dynamic database sources (TimeTree, Ensembl) archived with access date 2026-04-05. Source validation added to replication pipeline.
- v3.1.1 → v3.1.2: Reconciles summary counts with pipeline output: T2ok=25, Tmarg=22. Updates Fisher 2×2 table to 6 T3req vs 25 T2ok. No classification logic changes.
- v3.1 → v3.1.1: Corrects organism-level γ_crit computation (recalculated as T^(1/D) from stated D and T). Classification changes: C. elegans, S. cerevisiae, P. aeruginosa F15a T2ok → Tmarg. Adds organism-level Tmarg† policy. Corrects Section 2.10.2 family count. Family-level (F15b) classifications unchanged. Amphioxus TLR added to raw data. Full computational pipeline implemented.
- v2.0 → v3.1: Incorporates external pre-publication review (P1–P10, m1–m6). Key changes: amphioxus γ_crit corrected to 9.0 (elevated to Tier 1), zebrafish Hox and Arabidopsis RLKs reclassified to Tmarg, logistic regression replaced with Fisher exact test (p=1.36×10⁻⁶), bridge lemma added (§2.9), D distribution data presented (§2.10), γ definitions reconciled (§2.8.4), biological explanation of D~4.75 boundary added, neutral D_max~7 formalized, FP4 proof depth documented (Appendix C), two-regime γ elevated to Results. T3req list reduced from 8 to 6 families.
- v1.1 → v2.0: Incorporates reviewer feedback. Major changes: exact Lean theorem statements, F15a/F15b organism/family split, tier definition and selection section, cultural systems downgraded to Tmarg†, C. elegans reclassification, pan-genome basis removed from F15, empirical γ calibration added, brain cortical hierarchy replaced with gene-family basis, Linux D resolved to module nesting, WGD-adjusted D column added, adversarial cases added, amphioxus non-WGD control added, publication claims restricted to γ_crit ≤ 100.
